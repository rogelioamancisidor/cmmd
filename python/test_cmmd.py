...
from utils import weights_to_gpu,  mse, kl_divergence, marginal_kl, floatX
from CBMDxbmr import CBMD as CBMDxrmb
...
def test_cbmd(folder_name,idx=0):
    ...
    if args.dset == 'mnist':
        ...
        x1_dim = 28*28
        x2_dim = x1_dim
        _, _, _, x1_te, x2_te, y_te, _, _, _ = load_mnist()
    ...
    # extract weights
    weights_path = '../output/'+folder_name+'/best_weights_'+str(idx)+'.npy'
    #weights_path = '../output/'+folder_name+'/weights_'+str(idx)+'.npy'
    weights = np.load(weights_path)
    # order is enc(10), dec2(10), dec(10), cls(6)

    if args.dec_type == 'gaussian':
        enc_weights  = weights[0:10]    
        dec2_weights = weights[10:20] 
        ...
        dec2_weights = [weights_to_gpu(dec2_weights,args.nlayers_dec2,'Gaussian'),True]
    ...
    print 'creating CBMD model with pre-trained weights ...'
    if args.dset == 'xrmb_all':
        model = CBMDxrmb(x2_dim
                        ,x1_dim
                        ,args
                        ,y_dim
    ...
    if args.dset == 'mnist':
        ...
        print sum(np.sum(y_te != label_hat,axis=1)!=0)/float(y_te.shape[0])
    elif args.dset == 'flickr':
        print average_precision_score(y_te, model.draw_pi(x1_te), average='samples')
    elif args.dset in ('xrmb','xrmb_all'):
        speaker_avg =[]
        speakers = [7,16,20,21,23,28,31,35]
        
        _, _, _, x1_te, x2_te, y_te, _, _, _ = load_all_xrmb(idx_te=speakers)
        y_te = to_categorical(y_te,num_classes=y_dim)
        label_hat = model.y_pred(x1_te)
        print 'error rate for all speakers', np.mean(np.sum(y_te != label_hat,axis=1)!=0)

        _,id_te,_ = get_id_xrmb()
        for speaker in speakers:
            print 'error rate for speaker',speaker, ' is ', np.mean(np.sum(y_te[id_te[:,0]==speaker,:] != label_hat[id_te[:,0]==speaker,:],axis=1)!=0)
            x2_hat = model.draw_x2(x1_te[id_te[:,0]==speaker,:])
            print 'rmse speaker ',speaker,' is ', mse(x2_te[id_te[:,0]==speaker,:],x2_hat)
    elif args.dset == 'xrmb_folds':
        # posterior collapse
        y_te = to_categorical(y_te,num_classes=y_dim)
        z_mu, z_var = model.enc_params(x2_te,x1_te,y_te)
        mu0, var0   = model.prior_params(x1_te)
        ...
        _, x2_var = model.dec_params(x1_te)
        ...
        with open('../output/'+folder_name+'/cbmd'+str(r)+'_collapse1.pk','wb') as f:
            pickle.dump(dic1,f, protocol=pickle.HIGHEST_PROTOCOL)
        with open('../output/'+folder_name+'/cbmd'+str(r)+'_collapse2.pk','wb') as f:
            pickle.dump(dic2,f, protocol=pickle.HIGHEST_PROTOCOL)
        with open('../output/'+folder_name+'/cbmd'+str(r)+'_collapse3.pk','wb') as f:
            pickle.dump(dic3,f, protocol=pickle.HIGHEST_PROTOCOL)
        with open('../output/'+folder_name+'/cbmd'+str(r)+'_collapse4.pk','wb') as f:
        ...
        print 'rmse x2:', mse(x2_te,x2_hat)
...
def main():
    ...
    test_cbmd('xrmbfolds80')
    test_cbmd('xrmbfolds81')
    test_cbmd('xrmbfolds82')
    test_cbmd('xrmbfolds83')
